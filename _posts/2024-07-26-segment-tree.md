...
세그먼트 트리에서, query 함수로, **결합법칙이 성립** 하는 연산이 주어지는 경우, 적용할 수 있다. 결합법칙이 성립하는 연산이라는 것은, 

XOR, 곱셈, bitwise 연산, GCD, ... 등등이 있다. 
...
눈여겨볼 점은, 구간이 전혀 겹치지 않는 경우에는, 0을 반환한다는 사실이다. 여기서는, **구간 합** 을 구하고자 하는 것이기
